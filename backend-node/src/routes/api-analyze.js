...
router.post("/analyze", upload.single("video"), async (req, res) => {
  ...
  if (!userId || !sessionId || !topicId || !topicText || !turnIndex) {
    ...
    return fail(
      res,
      "userId, sessionId, topicId, topicText, and turnIndex are required",
    );
  }
  ...
  try {
    ...
    const scriptScore = Number(
      aiData?.scriptScore ??
        aiData?.textScore ??
        aiData?.modality_scores?.script ??
        aiData?.modality_scores?.text ??
        50,
    );
    ...
    const audioScore = Number(
      aiData?.audioScore ?? aiData?.modality_scores?.audio ?? 50,
    );
    const facialScore = Number(
      aiData?.facialScore ??
        aiData?.videoScore ??
        aiData?.modality_scores?.facial ??
        aiData?.modality_scores?.video ??
        50,
    );
    ...
    const finalScore = Number(
      aiData?.finalScore ??
        aiData?.sentiment_score ??
        aiData?.modality_scores?.final ??
        50,
    );

    const label = aiData?.sentiment_label ?? "neutral";
    const confidence = Number(
      aiData?.confidence ?? aiData?.modality_confidences?.final ?? 0.5,
    );
    ...
    const modalityWeights = {
      script: Number(
        aiData?.modality_weights?.script ??
          aiData?.modality_weights?.text ??
          0.4,
      ),
      audio: Number(aiData?.modality_weights?.audio ?? 0.3),
      facial: Number(
        aiData?.modality_weights?.facial ??
          aiData?.modality_weights?.video ??
          0.3,
      ),
    };
    const modalityConfidences = {
      script: Number(
        aiData?.modality_confidences?.script ??
          aiData?.modality_confidences?.text ??
          0,
      ),
      text: Number(
        aiData?.modality_confidences?.text ??
      ...
      facial: Number(
        aiData?.modality_confidences?.facial ??
          aiData?.modality_confidences?.video ??
          0,
      ),
      video: Number(
        aiData?.modality_confidences?.video ??
    ...
    const multimodalVersion =
      aiData?.multimodal_version ?? "video-audio-text-v2";

    const tasks = generateTasks(finalScore);
    const musicRecommendations = await getMusicRecommendations(
      finalScore,
    ...
    const relativeVideoPath = path
      .join("uploads", path.basename(videoFile.path))
      .replaceAll("\\", "/");
...
router.patch(
  "/analyze/:analysisId/tasks/:taskId/complete",
  async (req, res) => {
    const { analysisId, taskId } = req.params;
    console.log("Received request to complete task:", analysisId, taskId);
    ...
    session.currentMoodScore = Math.min(
      100,
      session.currentMoodScore + task.points,
    );
